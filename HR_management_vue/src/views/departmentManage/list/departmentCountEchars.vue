...
<script>
import {DepartmentcountEmployee} from  "@/api/departmentManage";
    export default {
    	  name: 'hello',
    	  data () {
    	    return {
    	  ...
    	  methods: {
			  ...
			  myEcharts(xDepartment,yCount) {
			     ...
			     initData() {
			       DepartmentcountEmployee()
			         .then(r => {
			           var xDepartment = [];
					   var yCount = [];
			           
			           r.forEach(e => {
			             xDepartment.push(e.name);
						 yCount.push(e.num);
			            
			           });
			           this.myEcharts(xDepartment,yCount);
			         })
			         .catch(() => {});
			     },
			     refresh(){
