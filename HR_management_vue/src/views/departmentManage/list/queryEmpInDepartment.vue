				...
				<el-container >
					<el-main style="height: 720px;">							
						<ToolBar>
							<div class="main-top">
								  <el-breadcrumb  >
									<el-breadcrumb-item :to="{ path: '/department_Manage/list' }">该部门下所有员工</el-breadcrumb-item >
...
<script>
import {queryEmpInDepartment} from  "@/api/departmentManage";
...
import {formatTimeToStr} from "@/config/date"; 
	 export default {	
		...
		methods:{
			...
			//查询部门下员工
			initEmptDate(){
				queryEmpInDepartment({departmentid:this.$route.query.departmentId})
				.then(r=>{
					this.tableData=r;
				})
			 .catch(()=>{});		
			...
			exportTable() {
			  ...
			  exportCvsTable(
				...
				//导出的表名
				this.tableData,
				this.$route.query.departmentId+"部门列表"
			  );
			},
